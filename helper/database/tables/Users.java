...
public class Users implements IUsers {
    ...
    public void addUser(String name, String status, String reason, int valueGrades, boolean isHelper,
        ...
        String insert = "INSERT INTO " + db.DB_FILE + " (name,helper,grades,status,muted," +
                "blacklist,active_question,reason,warns,cooldown)" +
                " VALUES (?,?,?,?,?,?,?,?,?,?)";
        ...
        preparedStatement.setInt(9, valueWarns);
        preparedStatement.setBoolean(10, isHaveCooldown);
    ...
    public List<String> getHelpersOnline() throws SQLException {
        rq.getRequest(db.getConnection(), "*", db.DB_FILE, null, RequestsType.ALL);
        return rq.getRsList();
    }
    ...
    public int getValueWarns(String name) throws SQLException {
        rq.getRequest(db.getConnection(), db.DB_FILE, "warns", name, RequestsType.INT);
        return rq.getRsInt();
    }
